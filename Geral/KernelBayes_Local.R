...
KernelBayes_Local = function (dados_norm_treino, dados_norm_teste,classes_treino, classes_teste, n_cluster, n_classes){
  ...
  for (i in 1:nrow(dados_norm_treino)){
    if(classes_treino[i]==classes_estimadas_treino[i]){
      corretas = corretas + 1
    } else if(classes_treino[i]!=classes_estimadas_treino[i]){
      erradas = erradas + 1
    }
  ...
  print(paste("INÍCIO do método Kernel Bayes Local - dados de TESTE:",Sys.time()))
  ...
  print("Bayes Local - teste")
  print(data.frame(erradas = erro_percentual_teste,
                   corretas = acerto_percentual_teste,
                   row.names = "percentual"))
  
  print(paste("FIM do método Kernel Bayes Local - dados de TESTE:",Sys.time()))
